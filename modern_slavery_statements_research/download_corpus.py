...
def get_aws_creds_from_session():
    ...
    :return: str aws_access_key_id, aws_secret_access_key
    """
...
def get_bucket_client(key_id, access_key):
    client = boto3.client('s3',
         aws_access_key_id=key_id,
         aws_secret_access_key=access_key)
    return client

def get_bucket_from_resource(bucket_name, key_id, access_key):
...
def get_latest_folder_name(bucket_name, key_id, access_key):
    """
    ...
    """
    # client = get_bucket_client(key_id, access_key)
...
def download(bucket_name='modern-slavery-dataset-txt', metadata_path='./metadatas.csv', only_metadata=False, key_id=None, access_key=None):
    ...
    if (not key_id or not access_key):
        key_id, access_key = get_aws_creds_from_session()

    remote_directory = get_latest_folder_name(bucket_name, key_id, access_key)
    bucket = get_bucket_from_resource(bucket_name, key_id, access_key)
    metadatas = []
...
if __name__ == '__main__':
    ...
    download(args.b, args.i, args.a, args.only_m, args.m)
